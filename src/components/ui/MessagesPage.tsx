    ...
    ws.onmessage = (evt: MessageEvent) => {
      try {
        ...
        if (
          ...
          msg.type === 'message' &&
          msg.data &&
          msg.data.booking_id === selectedBooking.id
        ) {
          setMessages((prev) => [
            ...prev,
            {
  ...
  return (
    <div className='h-full'>
      <div className='flex h-full'>
        ...
        <div
          className={`w-full md:w-80 bg-white border-r border-zinc-200 overflow-y-auto ${
            selectedBooking ? 'hidden md:block' : ''
          }`}
        >
          <div className='p-4 border-b border-zinc-200'>
            <h2 className='text-lg font-semibold text-zinc-900'>Conversas</h2>
            <p className='text-sm text-zinc-500'>
              {bookings.length} reservas ativas
            </p>
          </div>
                ...
                <div className='flex items-start gap-3'>
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <div className='flex items-center gap-4 text-xs text-zinc-400'>
                      <span>
                        {format(new Date(booking.check_in), 'dd/MM', {
                          locale: ptBR,
                        })}{' '}
                        -{' '}
                        {format(new Date(booking.check_out), 'dd/MM', {
                          locale: ptBR,
                        })}
                      </span>
                      <span>
                        R${' '}
                        {booking.total_price.toLocaleString('pt-BR', {
          ...
          {bookings.length === 0 && (
            <div className='p-8 text-center'>
              ...
              <p className='text-sm text-zinc-500'>
                As conversas aparecerão aqui quando houver reservas
              </p>
            </div>
          )}
        </div>
            ...
            <>
              ...
              <div className='bg-white border-b border-zinc-200 px-6 py-3'>
                <div className='flex items-center gap-6 text-sm'>
                  ...
                  <div className='flex items-center gap-2'>
                    <span className='text-zinc-600'>
                      {selectedBooking.number_of_guests} hóspedes
                    </span>
                  </div>
                  <div className='flex items-center gap-2'>
                    ...
                    <span className='text-zinc-600'>
                      R${' '}
                      {selectedBooking.total_price.toLocaleString('pt-BR', {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
              ...
              <div className='flex-1 overflow-y-auto p-6 space-y-4'>
                {messages.map((msg) => {
                  ...
                  try {
                    special = JSON.parse(msg.message) as PaymentInvite;
                  } catch {
                    special = null;
                  }
                  const isPaymentInvite = Boolean(
                    special && special.type === 'payment_invite'
                  );
                            ...
                            {isPaymentInvite ? (
                              <div className='space-y-2'>
                                ...
                                {special && special.checkout_url ? (
                                  ...
                                  >
                                    {String(
                                      (special && special.cta) || 'Pagar agora'
                                    )}
                                  </a>
                                ) : (
            ...
            <div className='flex-1 flex items-center justify-center bg-zinc-50'>
              <div className='text-center'>
                ...
                <p className='text-zinc-500'>
                  Escolha uma reserva ao lado para começar a conversar
                </p>
              </div>
            </div>
          )}
