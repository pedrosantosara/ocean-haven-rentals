...
func BookingAcceptedTemplate(data BookingEmailData) string {
    body := fmt.Sprintf(`<p>Olá, %s!</p><p>Sua hospedagem foi confirmada.</p><div class="box"><div><strong>Check-in:</strong> %s</div><div><strong>Check-out:</strong> %s</div><div><strong>Hóspedes:</strong> %d</div><div><strong>Total:</strong> R$ %.2f</div></div><p>Prepare-se para dias de praia e conforto.</p>`, data.Name, data.CheckIn, data.CheckOut, data.Guests, data.TotalPrice)
    return baseTemplate("Reserva confirmada", body)
}

func ChatNotificationTemplate(data ChatMessageEmailData) string {
    body := fmt.Sprintf(`<p>Nova mensagem de %s:</p><div class="box"><div style="white-space:pre-wrap">%s</div><div class="muted" style="margin-top:8px">%s</div></div><p>Responda pelo chat para continuar a conversa.</p>`, data.SenderName, data.Message, data.Timestamp)
    return baseTemplate("Nova mensagem no chat", body)
}